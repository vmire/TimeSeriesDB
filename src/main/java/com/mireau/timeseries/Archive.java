 ...
 *   	En-tête spécifique : Données sur le step en cours (28 bytes)
...
public abstract class Archive {
	 ...
	 * @throws ArchiveInitException
	 */
	protected Archive(File file, String id, Integer step, WriteStrategy strategy) throws IOException, ArchiveInitException{
		this(file,id,step);
	...
	public static Archive getArchive(File file, String id) throws IOException, ArchiveInitException{
		...
		f.close();
		if(type == Type.AVERAGE) archive = new AverageArchive(file, id,step);
		else if(type == Type.ABS_COUNTER) archive = new AbsCounterArchive(file, id,step);
		return archive;
	}
	 ...
	 * @throws ArchiveInitException
	 */
	protected static void newArchiveFile(Integer step, Archive.Type type, File file) throws IOException, ArchiveInitException{
	 ...
	 * @throws IOException 
	 */
	public static Archive getArchive(File file, String id, Integer step, Type type) throws IOException, ArchiveInitException{
		Archive archive = null;
		if(type == Type.AVERAGE) archive = new AverageArchive(file, id, step);
		else if(type == Type.ABS_COUNTER) archive = new AbsCounterArchive(file, id, step);
		return archive;
	...
	public Type getType() {
		if (this instanceof AverageArchive) return Type.AVERAGE;
		else if (this instanceof AbsCounterArchive) return Type.ABS_COUNTER;
		else return null;
	}
	 ...
	 * @return
	 */
	protected abstract int enregLen();
